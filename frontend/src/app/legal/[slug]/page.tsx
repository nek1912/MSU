...
import { getLegalDoc, slugAccent, legalDocs as rawLegalDocs } from "@/lib/data";
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
      <div className="rounded-xl p-6 text-white" style={{ backgroundColor: accent }}>
        <Badge tone="neutral">{t(`legalCategory.${sc.category}`)}</Badge>
        <h1 className="mt-2 font-[var(--font-display)] text-[var(--text-2xl)] font-[var(--font-medium)] tracking-tight">{sc.title}</h1>
        <p className="mt-1 text-sm opacity-90">{sc.overview}</p>
      ...
      <div className="mt-6 space-y-6">
        <section className="rounded-[var(--radius-xl)] bg-[var(--surface-elevated)] p-[var(--space-6)]">
          ...
          <p className="mt-2 text-sm text-[var(--text-secondary)]">{sc.overview}</p>
        </section>
        {SECTIONS.map((s) => (
