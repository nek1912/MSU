# Phase 2A MVP Ingestion Implementation Plan (v3)
...
actual_title: string (required)
issuing_organization: string (required)
target_domain: string (required)
jurisdiction: string (required, "central" or "state")
state: string|null (required if jurisdiction="state")
document_type: string (required)
official_source_url: string (required)
effective_date: string|null (required)
document_date: string|null (required)
...
MANIFEST_REQUIRED_FIELDS = [
    ...
    "effective_date", "document_date", "verified_date",
]
...
def manifest_to_supabase(manifest_path: Path, embed_texts, supabase, dry_run: bool = False, preflight: bool = False) -> int:
    ...
    for source in valid_sources:
        ...
        try:
            ...
            # Generate embeddings (for preflight or actual ingestion)
            ...
            # Per-file error isolation: continue with next file
            continue
...
def test_preflight_does_not_write_to_db():
    ...
    count = manifest_to_supabase(MANIFEST_PATH, mock_embed, mock_supabase, preflight=True)
    assert count > 0
    # Verify no DB writes occurred
    mock_supabase.table.return_value.insert.return_value.execute.assert_not_called()
...
BEGIN
  ...
  IF p_chunks_data IS NULL OR array_length(p_chunks_data, 1) = 0 THEN
    RAISE EXCEPTION 'No chunks provided for document %', p_source_id;
  END IF;
...
def test_rpc_vector_serialization():
    """Test that vector serialization works with actual Supabase."""
    from app.db import get_supabase
    
    supabase = get_supabase()
    
    # Test vector serialization format
    test_vector = [0.1] * 768
    vector_str = str(test_vector)
    
    # Verify the format is accepted by PostgreSQL
    # This is a minimal test - the actual RPC will validate
    assert isinstance(vector_str, str)
    assert len(vector_str) > 0
...
def test_rpc_atomicity_rollback():
    ...
    # Try to replace with invalid data (should fail)
    try:
        atomic_replace_document(
            supabase,
            source_id="test_rollback_doc",
            doc_data={"title": "Test Rollback"},
            chunks_data=[]  # Empty chunks should fail
        )
    except Exception:
        pass
...
```bash
git add backend/migrations/0001_init.sql ingestion/ingestion/ingest.py ingestion/tests/test_atomic_replacement.py ingestion/tests/test_rpc_atomicity.py
